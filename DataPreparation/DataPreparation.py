...
for col in num_cols:
    ...
    dataset[f'{col}_outlier_z'] = z_scores > 3
    
    # IQR method
    Q1 = dataset[col].quantile(0.25)
    Q3 = dataset[col].quantile(0.75)
    IQR = Q3 - Q1
    dataset[f'{col}_outlier_iqr'] = (dataset[col] < (Q1 - 1.5 * IQR)) | (dataset[col] > (Q3 + 1.5 * IQR))
    
    outliers[col] = {
        'z_score_outliers': dataset[f'{col}_outlier_z'].sum(),
        'iqr_outliers': dataset[f'{col}_outlier_iqr'].sum()
    }

logging.info(f"Outlier detection completed: {outliers}")

dataset.to_csv(f"Staging/OUT/telecom_customer_cleaned_dataset_{date_time}.csv", index=False)
...
for col in num_cols:
    plt.figure()
    dataset[col].hist(figsize=(10, 8), bins=30)
